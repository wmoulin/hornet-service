...
public class ClassType extends AbstractNamedType {
    ...
    private final Map<String, FunctionType> methods = new LinkedHashMap<String, FunctionType>();
    ...
    @Override
    public void writeDef(final Writer writer) throws IOException {

        writer.write(format("interface %s {\n", name));
        for (final Entry<String, AbstractType> entry : fields.entrySet()) {
            writer.write(format("    %s?: ", entry.getKey()));
            entry.getValue().write(writer);
            writer.write(";\n");
        }
        for (final String methodName : methods.keySet()) {
            final FunctionType method = this.methods.get(methodName);
            if (method.getUri() != null) {
                writer.write("    // " + method.getUri() + "\n");
            }
    ...
    public void setFields(final Map<String, AbstractType> fields) {
